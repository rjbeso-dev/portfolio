...
export const projects: Project[] = [
  ...
  },
  {
    ...
    category: "AI Automation",
    problem:
      "Founders lose hours a day to two things: triaging a flooded inbox, and turning meetings into decisions and action items that don't get lost. Both are ideal for AI, but only if the system is reliable. Most automations misclassify silently or mangle output, and people quietly stop trusting them. This is a multi-pipeline operator system that handles both, with confidence-aware routing and defensive design at every layer.",
    approach:
      "Two n8n pipelines. The INBOX workflow (9 nodes) takes inbound email, Claude Haiku 4.5 classifies each into 5 categories with confidence and reasoning, a Switch routes Respond Needed items through a second Claude call that drafts three reply variations (formal, casual, direct), and structured rows are written to Notion. The MEETING workflow turns a meeting into structured output: OpenAI Whisper transcribes the audio, then a single Claude extraction call produces decisions and action items that fan out in parallel into a Notion Decisions Log and an Action Items database (task, owner, due date, priority). Both are wrapped in five layers of defensive design: HTTP rate-limit batching, markdown code-fence stripping on every LLM response, a confidence-threshold fallback, cross-branch data preservation via n8n pairing, and Notion select-option normalisation. Synthetic fixtures with ground-truth labels validate accuracy on every run.",
    outcome:
      "On the inbox side: 20 emails processed in about 50 seconds at roughly $0.015 per run, with three sendable reply variations per item in the founder's voice. On the meeting side: a recording goes in and a structured Decisions Log plus assigned Action Items come out, so follow-ups stop falling through the cracks. Eight distinct n8n quirks debugged from production-style errors during the build, each captured as a worked example of senior-level diagnosis. Nudge Bot and Weekly Brief in progress.",
    year: "2026",
    status: "Case study",
    href: "#",
    tags: ["n8n", "Claude Haiku 4.5", "OpenAI Whisper", "Notion API", "JavaScript", "Meeting automation", "Multi-LLM orchestration", "Defensive design"],
    screenshots: [
      {
